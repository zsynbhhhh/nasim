...
class PolicyNet(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim, action_bound):
        ...
        self.action_bound = action_bound  # action_bound 是环境可以接受的最大动作值

    def forward(self, x):
        x = F.relu(self.fc1(x))
        return torch.tanh(self.fc2(x)) * self.action_bound # tanh可以用来使得数值为-1, 1, self.action_bound可以将其变为对应的离散范围
...
class DDPG:
    ...
    def __init__(self, num_in_actor, num_out_actor, num_in_critic, hidden_dim,
                 ...
                 gamma, device):
        out_fn = (lambda x: x) if discrete else (  # 如果是分离的或者连续的
        ...
        self.action_bound = action_bound # action_bound 是环境可以接受的动作的最大值
    ...
    def update(self, transition_dict):
        ...
        actor_loss = -torch.mean(
            self.critic(
                # 策略网络就是为了使得Q值最大值
                torch.cat([states, self.actor(states)], dim=1))) # 为了使得Q值最大
